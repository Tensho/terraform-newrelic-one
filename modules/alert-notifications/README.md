...
| <a name="input_workflows"></a> [workflows](#input\_workflows) | Notification workflows. | <pre>map(object({<br/>    name                  = string<br/>    muting_rules_handling = string<br/><br/>    issues_filter = object({<br/>      name = string<br/><br/>      predicates = list(object({<br/>        attribute = string<br/>        operator  = string<br/>        values    = list(string)<br/>      }))<br/>    })<br/><br/>    notification_channels = map(object({<br/>      name = string<br/>      type = string<br/><br/>      # Reference a destination in the same module (destination_key)<br/>      destination_key = optional(string)<br/>      # Rereference a destination in other module (destination_id)<br/>      destination_id = optional(string)<br/><br/>      properties = optional(list(object({<br/>        key           = string<br/>        value         = string<br/>        label         = optional(string)<br/>        display_value = optional(string)<br/>      })), [])<br/>    }))<br/>  }))</pre> | `{}` | no |
